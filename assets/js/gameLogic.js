...
const game = {
    ...
    configPontuacaoPorDificuldade: {}, // NOVO: Para armazenar as pontuações configuradas
    ...
    async setupGame(playerNamesArray, numTotalCasas, configEspeciais, configPerguntas,
                    componentesFiltro, conteudosFiltro,
                    dificuldadeFiltro, rawQuestions,
                    permitePontuacaoNegativaConfig, tempoLancamentoDadoSeg,
                    anosTurmasFiltro, bimestresFiltro,
                    pontuacaoConfigurada) { // NOVO PARÂMETRO
        ...
        // Se você estiver usando tempPlayersData (com avatares), ajuste esta linha:
        this.players = playerNamesArray.map(data =>
            typeof data === 'object' ? new Player(data.name, this.configPermitePontuacaoNegativa, data.avatarSrc) : new Player(data, this.configPermitePontuacaoNegativa)
        );
        // Embaralhar jogadores...

        this.configPontuacaoPorDificuldade = pontuacaoConfigurada; // Armazena a configuração
        console.log("[GameLogic] Pontuação por dificuldade configurada:", this.configPontuacaoPorDificuldade);
        ...
        console.log("[GameLogic] Filtro de Componente Recebido:", componentesFiltro);
        console.log("[GameLogic] Filtro de Conteúdo (Categoria) Recebido:", conteudosFiltro);


        // 1. Filtro de Componente Curricular (NOVO - APLICADO PRIMEIRO)
        if (componentesFiltro && Array.isArray(componentesFiltro) && !componentesFiltro.includes('todos_componente') && componentesFiltro.length > 0) {
            perguntasFiltradas = perguntasFiltradas.filter(q => q.componente_curricular && componentesFiltro.includes(q.componente_curricular));
        }
        console.log("[GameLogic] Perguntas após filtro de Componente:", perguntasFiltradas.length);

        // 2. Filtro de Conteúdo Específico (Antiga Categoria)
        if (conteudosFiltro && Array.isArray(conteudosFiltro) && !conteudosFiltro.includes('todos_conteudo') && conteudosFiltro.length > 0) {
            perguntasFiltradas = perguntasFiltradas.filter(q => q.categoria && conteudosFiltro.includes(q.categoria));
        }
        console.log("[GameLogic] Perguntas após filtro de Conteúdo (Categoria):", perguntasFiltradas.length);
        ...
        // 5. Filtro de Bimestre
        if (bimestresFiltro && Array.isArray(bimestresFiltro) && !bimestresFiltro.includes('todos_bimestre') && bimestresFiltro.length > 0) {
            perguntasFiltradas = perguntasFiltradas.filter(q => q.bimestre && bimestresFiltro.includes(q.bimestre));
        }
        ...
        console.log(`Jogo configurado com ${this.gameQuestions.length} perguntas. Filtros aplicados: Componentes=${componentesFiltro.join(',')}, Conteúdos=${conteudosFiltro.join(',')}, Dificuldade=${dificuldadeFiltro}, Anos/Turmas=${anosTurmasFiltro.join(',')}, Bimestres=${bimestresFiltro.join(',')}`);
        console.log(`Jogo configurado com ${this.gameQuestions.length} perguntas e pontuações:`, this.configPontuacaoPorDificuldade);
    },
            ...
            case 'pergunta':
                ...
                if (question) {
                    ui.showQuestionModal(question, (isCorrect) => {
                        ...
                        if (isCorrect) {
                            // MODIFICADO: Usar a pontuação configurada
                            let pontosGanhos = question.pontos; // Fallback para o valor da pergunta
                            if (this.configPontuacaoPorDificuldade &&
                                this.configPontuacaoPorDificuldade.hasOwnProperty(question.dificuldade)) {
                                pontosGanhos = this.configPontuacaoPorDificuldade[question.dificuldade];
                                console.log(`[GameLogic] Usando pontuação configurada para dificuldade '${question.dificuldade}': ${pontosGanhos}`);
                            } else {
                                console.log(`[GameLogic] Usando pontuação da pergunta para dificuldade '${question.dificuldade}' (fallback): ${pontosGanhos}`);
                            }

                            player.addPoints(pontosGanhos);
                            feedbackMessage = `${player.name} ganhou ${pontosGanhos} pontos!`;
                        } else {
                            // A lógica de perder pontos pode permanecer a mesma ou também ser configurável
                            const pontosPerdidos = Math.floor(Math.random() * (Math.floor((this.configPontuacaoPorDificuldade[question.dificuldade] || question.pontos) / 3) || 1)) + 1;
                            player.losePoints(pontosPerdidos);
